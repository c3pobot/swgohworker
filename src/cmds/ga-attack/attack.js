...
module.exports = async(obj = {}, opt = [], mode = '5v5')=>{
  try{
    ...
    let gl_only = await HP.GetOptValue(opt, 'gl_only', false)
    ...
    if(tempSquads?.length > 0){
      ...
      if(squads?.length > 0){
        ...
        info.header = 'GAC Season '+countSquads?._id+' '+countSquads?.mode+' '+unitList[leader].name+' Who to attack in '+countSquads?.total+' battles'
      }
    }
